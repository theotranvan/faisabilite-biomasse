...
/**
 * Calculate cost for a year given initial cost and annual increase rate
 * coutAnnee_N = coutAnnee_1 × (1 + taux)^(N-1)
 */
export function calculCoutYear(
  coutInitial: number,
  tauxAugmentation: number,
  year: number
): number {
  return coutInitial * Math.pow(1 + tauxAugmentation, year - 1);
}

/**
 * Calculate cost with amortization deduction for year 16+ (after loan period ends)
 */
export function calculCoutAnneeAnnuiteEnded(
  coutAnnee15: number,
  tauxAugmentation: number,
  annuite: number,
  year: number
): number {
  const coutAnneeN = coutAnnee15 * Math.pow(1 + tauxAugmentation, year - 15);
  return coutAnneeN - annuite;
}

/**
 * Build 20-year cost projection
 */
export function calculBilan20Ans(
  ...
  for (let year = 1; year <= 20; year++) {
    let coutActuel: number;
    let coutRef: number;
    let coutBiomasse: number;

    if (year <= dureeEmprunt) {
      // Years 1-15: with amortization
      coutActuel = calculCoutYear(coutInitialActuel, tauxAugmentationfossile, year);
      coutRef = calculCoutYear(coutInitialRef, tauxAugmentationfossile, year);
      coutBiomasse = calculCoutYear(coutInitialBiomasse, tauxAugmentationBiomasse, year);
    } else {
      // Years 16-20: after loan ends (subtract amortization)
      const coutActuel15 = calculCoutYear(coutInitialActuel, tauxAugmentationfossile, dureeEmprunt);
      const coutRef15 = calculCoutYear(coutInitialRef, tauxAugmentationfossile, dureeEmprunt);
      const coutBiomasse15 = calculCoutYear(coutInitialBiomasse, tauxAugmentationBiomasse, dureeEmprunt);

      coutActuel = calculCoutAnneeAnnuiteEnded(
        coutActuel15,
        tauxAugmentationfossile,
        0, // No amortization for current scenario
        year
      );
      coutRef = calculCoutAnneeAnnuiteEnded(
        coutRef15,
        tauxAugmentationfossile,
        annuiteRef,
        year
      );
      coutBiomasse = calculCoutAnneeAnnuiteEnded(
        coutBiomasse15,
    ...
    const economie = coutRef - coutBiomasse;
